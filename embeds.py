...
def _match_no(match: dict) -> str:
    """Match number is now shown as a subtle suffix in the stage badge, not in titles.

    Kept for compatibility — always returns empty string so existing callers
...
def _stage_badge(match: dict) -> str:
    ...
    Format (group stage):
        Group Stage · Group A · Matchday 2 · Tournament Day 7  ·  #42
    Matchday = which of the 3 group-stage rounds this is for that group.
    Tournament Day = calendar day number since the WC opening match.
    Match number comes last as a low-emphasis reference.
    """
    ...
    # Matchday (1/2/3) only shown in group stage
    matchday = match.get("matchDay") or match.get("matchday")
    if matchday and stage in ("GROUP_STAGE", ""):
        label += f" · Matchday {matchday}"
    # Tournament Day: how far into the competition (Day 1 = Jun 11 2026)
    comp_day = match.get("competitionDay")
    if comp_day and comp_day >= 1:
        label += f" · Day {comp_day}"
...
def embed_halftime(match: dict, detail: dict) -> discord.Embed:
    ...
    hf   = team_flag(match.get("homeTeam", {}))
    af   = team_flag(match.get("awayTeam", {}))
    # Try halfTime period first; fall back to current score if null (API lag)
    h, a = get_score(detail, "halfTime") if detail else (None, None)
    if h is None or a is None:
        h, a = get_current_score(detail or match)
    # Ensure we never show Python's None — minimum is 0-0
...
def embed_yellow_card(match: dict, detail: dict, card: dict) -> discord.Embed:
    home   = team_display(match.get("homeTeam", {}))
    away   = team_display(match.get("awayTeam", {}))
    hf     = team_flag(match.get("homeTeam", {}))
    af     = team_flag(match.get("awayTeam", {}))
...
def embed_substitution(match: dict, detail: dict, sub: dict) -> discord.Embed:
    home   = team_display(match.get("homeTeam", {}))
    away   = team_display(match.get("awayTeam", {}))
    hf     = team_flag(match.get("homeTeam", {}))
    af     = team_flag(match.get("awayTeam", {}))
    ...
    player_in = sub.get("player_in") or sub.get("playerIn") or sub.get("in") or "Player on"
    player_out = sub.get("player_out") or sub.get("playerOut") or sub.get("out") or "Player off"

    em = discord.Embed(
        title="🔁 Substitution",
        description=(
            f"**{player_in}** on\n"
            f"{player_out} off  ·  {minute}'"
        ),
        color=C_BLUE,
        timestamp=datetime.now(timezone.utc),
    )
    em.add_field(name="Match", value=f"{_match_no(match)}{hf} {home}  vs  {away} {af}", inline=False)
    if team:
        em.add_field(name="Team", value=team, inline=True)
    em.add_field(name="Minute", value=f"{minute}'", inline=True)
    return _footer(em, "• Substitution")
...
def embed_lineups(match: dict, detail: dict) -> discord.Embed:
    ...
    for lineup in lineups[:2]:
        ...
        for entry in players:
            ...
            # football-data.org v4 returns full position words, not abbreviations.
            # Both full words and abbreviations are mapped so the embed is correct
            # regardless of API tier or future changes.
            pos_map = {
                # Full words returned by football-data.org v4 API
                "Goalkeeper": "🟡", "Keeper": "🟡",
                "Defence":    "🔵", "Defender":  "🔵",
                "Midfield":   "🟢", "Midfielder": "🟢",
                "Offence":    "🔴", "Attacker":   "🔴", "Forward": "🔴",
                # Abbreviations (fallback / other providers)
                "GK": "🟡", "DEF": "🔵", "MID": "🟢", "FWD": "🔴",
            }
            icon   = pos_map.get(pos, "⚪")
            number = f"`{shirt}` " if shirt is not None else ""
            player_names.append(f"{icon} {number}{name}")
...
def embed_motm_vote(match: dict, nominees: list[str] | None = None) -> discord.Embed:
    home  = team_display(match.get("homeTeam", {}))
    away  = team_display(match.get("awayTeam", {}))
    hf    = team_flag(match.get("homeTeam", {}))
    af    = team_flag(match.get("awayTeam", {}))

    # Build nominee shortlist text if provided
    if nominees:
        nominee_lines = "\n".join(f"⭐ {n}" for n in nominees)
    ...
    em = discord.Embed(
        ...
        description=(
            f"**{_match_no(match)}{hf} {home}  vs  {away} {af}**"
            f"{shortlist_text}\n"
            "Who was the standout player? Cast your vote using the dropdown below!\n"
...
def embed_motm_result(
    match: dict,
    winners: list[str],
    tally: dict,
    is_official: bool = False,
) -> discord.Embed:
    """req #4/#14: is_official=True labels the result as the official API winner."""
    home = team_display(match.get("homeTeam", {}))
    away = team_display(match.get("awayTeam", {}))
    hf   = team_flag(match.get("homeTeam", {}))
    af   = team_flag(match.get("awayTeam", {}))

    source_label = "🏆 Official Man of the Match" if is_official else "🌟 Community Man of the Match"
    ...
    if is_official:
        em.set_footer(text="✅ Official match award  •  🏆 FIFA World Cup 2026")
    else:
        em.set_footer(text="🗳️ Based on community votes  •  🏆 FIFA World Cup 2026")
...
def embed_wc_group(letter: str, rows: list[dict]) -> discord.Embed:
    ...
    for entry in rows:
        ...
        pts    = entry.get("points", 0)
        adv    = "✅" if pos <= 2 else "▪️"
        stat   = f"`{played}` **{won}**W `{draw}`D `{lost}`L"
        goals  = f"`{gf}:{ga}` `{gd:+d}`"
        lines.append(
            f"{adv} **{pos}.** {flag} **{name}**\n"
            f"  {stat}  {goals}  **{pts} pts**"
        )
...
def embed_full_recap(
    match: dict,
    detail: dict | None,
    youtube_url: str | None,
    match_stats: dict | None = None,
) -> discord.Embed:
    """Comprehensive 1-hour post-match report.

    Parameters
    ----------
    match:        raw match dict (may be annotated)
    detail:       full match detail from get_match_detail()
    youtube_url:  optional YouTube highlights URL
    match_stats:  optional structured stats dict from get_match_stats() —
                  provides pre-split home/away goals, cards, subs, lineups
    """
    home  = team_display(match.get("homeTeam", {}))
    away  = team_display(match.get("awayTeam", {}))
    hf    = team_flag(match.get("homeTeam", {}))
    ...
    # ── Header ────────────────────────────────────────────────────────────────
    score_duration = ""
    score_obj = (detail or match).get("score") or {}
    duration = score_obj.get("duration", "REGULAR")
    if duration == "EXTRA_TIME":
        score_duration = "  *(AET)*"
    elif duration == "PENALTY_SHOOTOUT":
        pen = score_obj.get("penalties") or {}
        hp, ap = pen.get("home"), pen.get("away")
        if hp is not None and ap is not None:
            score_duration = f"  *(AET · Pen: {hp}–{ap})*"
        else:
            score_duration = "  *(On Penalties)*"

    em = discord.Embed(
        title=f"📺 Match Report  ·  {stage}",
        description=(
            f"## {_match_no(match)}{hf} {home}  **{h} – {a}**  {away} {af}{score_duration}\n"
            "*Full-time report — comprehensive match statistics*"
        ),
        color=C_BLUE,
    )
    ...
    # ── Use pre-split stats when available (from get_match_stats) ─────────────
    if match_stats:
        home_goals   = match_stats.get("home_goals",   []) or []
        away_goals   = match_stats.get("away_goals",   []) or []
        home_yellows = match_stats.get("home_yellows", []) or []
        away_yellows = match_stats.get("away_yellows", []) or []
        home_reds    = match_stats.get("home_reds",    []) or []
        away_reds    = match_stats.get("away_reds",    []) or []
        home_subs    = match_stats.get("home_subs",    []) or []
        away_subs    = match_stats.get("away_subs",    []) or []
        home_lineup  = match_stats.get("home_lineup",  {}) or {}
        away_lineup  = match_stats.get("away_lineup",  {}) or {}
        all_bookings = match_stats.get("bookings",     []) or []
    else:
        # Fall back: split from detail directly
        all_goals    = d.get("goals",    []) or []
        all_bookings = d.get("bookings", []) or []
        all_subs     = d.get("substitutions") or []
        lineups      = d.get("lineups") or []

        def _tid(entry: dict) -> object:
            return (entry.get("team") or {}).get("id")

        home_goals   = [g for g in all_goals    if _tid(g) == home_id]
        away_goals   = [g for g in all_goals    if _tid(g) != home_id]
        home_yellows = [b for b in all_bookings if _tid(b) == home_id and b.get("card") == "YELLOW"]
        away_yellows = [b for b in all_bookings if _tid(b) != home_id and b.get("card") == "YELLOW"]
        home_reds    = [b for b in all_bookings if _tid(b) == home_id and b.get("card") in ("RED", "YELLOW_RED")]
        away_reds    = [b for b in all_bookings if _tid(b) != home_id and b.get("card") in ("RED", "YELLOW_RED")]
        home_subs    = [s for s in all_subs     if _tid(s) == home_id]
        away_subs    = [s for s in all_subs     if _tid(s) != home_id]
        home_lineup  = next((l for l in lineups if (l.get("team") or {}).get("id") == home_id), {})
        away_lineup  = next((l for l in lineups if (l.get("team") or {}).get("id") != home_id and l), {})

    # ── Goal timeline ──────────────────────────────────────────────────────────
    def _goal_lines(goals_list: list) -> str:
        lines = []
        for g in goals_list:
            scorer = (g.get("scorer") or {}).get("name", "OG")
            assist = (g.get("assist") or {}).get("name")
            minute = g.get("minute", "?")
            gt     = str(g.get("type", "")).upper()
            tag    = " *(Pen)*" if gt == "PENALTY" else (" *(OG)*" if gt == "OWN" else "")
            ast    = f"  ↳ *{assist}*" if assist else ""
            lines.append(f"⚽ **{scorer}**{tag}  {minute}'{ast}")
        return "\n".join(lines) if lines else "—"

    em.add_field(name=f"{hf} {home} Goals", value=_goal_lines(home_goals), inline=True)
    em.add_field(name=f"{af} {away} Goals", value=_goal_lines(away_goals), inline=True)
    em.add_field(name="\u200b", value="\u200b", inline=False)

    # ── Discipline (cards) ────────────────────────────────────────────────────
    def _card_lines(yellows: list, reds: list, flag: str, name: str) -> str:
        lines = []
        for b in yellows:
            p = (b.get("player") or {}).get("name", "?")
            m = b.get("minute", "?")
            lines.append(f"🟨 {p}  {m}'")
        for b in reds:
            p = (b.get("player") or {}).get("name", "?")
    ...
    total_yellows = len(home_yellows) + len(away_yellows)
    total_reds    = len(home_reds)    + len(away_reds)
    if total_yellows or total_reds:
        em.add_field(
            name=f"{hf} {home} Discipline",
            value=_card_lines(home_yellows, home_reds, hf, home),
            inline=True,
        )
        em.add_field(
            name=f"{af} {away} Discipline",
            value=_card_lines(away_yellows, away_reds, af, away),
            inline=True,
        )
        em.add_field(name="\u200b", value="\u200b", inline=False)

    # ── Substitutions ─────────────────────────────────────────────────────────
    def _sub_lines(subs: list) -> str:
        lines = []
        for s in subs:
            p_in  = (s.get("playerIn")  or s.get("player_in")  or {})
            p_out = (s.get("playerOut") or s.get("player_out") or {})
            name_in  = p_in.get("name",  p_in)  if isinstance(p_in,  dict) else str(p_in)
            name_out = p_out.get("name", p_out) if isinstance(p_out, dict) else str(p_out)
            minute   = s.get("minute", "?")
            if name_in or name_out:
                lines.append(f"🔄 {name_in} ↑ / {name_out} ↓  {minute}'")
        return "\n".join(lines[:8]) if lines else "None"

    if home_subs or away_subs:
        em.add_field(
            name=f"{hf} {home} Substitutions ({len(home_subs)})",
            value=_sub_lines(home_subs),
            inline=True,
        )
        em.add_field(
            name=f"{af} {away} Substitutions ({len(away_subs)})",
            value=_sub_lines(away_subs),
            inline=True,
        )
        em.add_field(name="\u200b", value="\u200b", inline=False)

    # ── Lineups ───────────────────────────────────────────────────────────────
    def _lineup_str(lu: dict) -> str:
        formation = lu.get("formation", "")
        xi        = lu.get("startXI") or lu.get("startingXI") or []
        names     = []
        for entry in xi[:11]:
            if isinstance(entry, dict):
                p = entry.get("player") or entry
                names.append(p.get("name", "?") if isinstance(p, dict) else str(p))
        bench_raw = lu.get("bench") or lu.get("substitutes") or []
        bench = [
            (e.get("player") or e).get("name", "?") if isinstance(e, dict) else str(e)
            for e in bench_raw[:7]
        ]
        parts = []
        if formation:
            parts.append(f"**Formation:** {formation}")
        if names:
            parts.append("**Starting XI:** " + " · ".join(names))
        if bench:
            parts.append("**Bench:** " + " · ".join(bench))
        return "\n".join(parts) if parts else "Not available"

    if home_lineup or away_lineup:
        em.add_field(
            name=f"{hf} {home} Lineup",
            value=_lineup_str(home_lineup)[:512],
            inline=False,
        )
        em.add_field(
            name=f"{af} {away} Lineup",
            value=_lineup_str(away_lineup)[:512],
            inline=False,
        )

    # ── API-provided match statistics (possession, shots, xG etc.) ────────────
    # football-data.org includes these in detail["statistics"] on paid plans;
    # we render them when present and silently skip when absent (free-tier).
    raw_stats = d.get("statistics") or d.get("stats") or []
    if isinstance(raw_stats, list) and raw_stats:
        stat_labels = {
            "ball_possession":         ("🏟️", "Possession"),
        ...
        for entry in raw_stats:
            stat_type = str(entry.get("type") or "").lower().replace(" ", "_")
            home_val  = entry.get("home")
            away_val  = entry.get("away")
            if home_val is None and away_val is None:
                continue
            icon, label = stat_labels.get(stat_type, ("📊", stat_type.replace("_", " ").title()))
            stat_lines.append(f"{icon} **{label}**: `{home_val}` — `{away_val}`")
        if stat_lines:
            em.add_field(
                name=f"📊 Match Statistics  ({hf} vs {af})",
                value="\n".join(stat_lines[:20]),
                inline=False,
            )

    # ── Match summary line counts ──────────────────────────────────────────────
    summary_parts = []
    if total_yellows:
        summary_parts.append(f"🟨 {total_yellows} yellow{'s' if total_yellows != 1 else ''}")
    if total_reds:
        summary_parts.append(f"🟥 {total_reds} red{'s' if total_reds != 1 else ''}")
    if (home_subs or away_subs):
        total_subs = len(home_subs) + len(away_subs)
        summary_parts.append(f"🔄 {total_subs} sub{'s' if total_subs != 1 else ''}")
    if summary_parts:
        em.add_field(name="📋 Match Summary", value="  ·  ".join(summary_parts), inline=False)
...
def embed_status(guild: Any, cfg: dict, monitor_running: bool) -> discord.Embed:
    ...
    em.add_field(name="🎯 Predictions & Results", value=_ch("predictions_channel_id"), inline=True)
...
def embed_setup_intro() -> discord.Embed:
    em = discord.Embed(
        ...
        description=(
            ...
            "2️⃣ Predictions & Results channel *(polls, MOTM votes, leaderboard)*\n"
            "3️⃣ Match recaps channel\n"
            "4️⃣ Timezone\n"
            "5️⃣ Notification mode\n\n"
...
def embed_setup_complete(cfg: dict, guild: Any) -> discord.Embed:
    ...
    em.add_field(name="🎯 Predictions & Results", value=_ch("predictions_channel_id"), inline=True)
    em.add_field(name="📊 Recaps",                value=_ch("summary_channel_id"),     inline=True)
    em.add_field(name="🔔 Mode",                  value=cfg.get("mode", "detailed"),   inline=True)
...
# ── All-groups overview embed (req #10) ───────────────────────────────────────

def embed_all_groups_standings(standings_data: dict) -> discord.Embed:
    """req #10: Single embed with all group tables as inline fields.

    Designed to be pinned as the authoritative group-stage overview.
    Each field = one group (Group A … Group H), inline for compact layout.
    """
    em = discord.Embed(
        title="🏆 FIFA World Cup 2026 — Group Stage Standings",
        color=C_GOLD,
    )
    ...
    for table in (standings_data.get("standings") or []):
        ...
        for row in rows:
            ...
            lines.append(f"{flag} {name} · P{played} · {pts}pts · {gd_str}")

        field_value = "\n".join(lines) if lines else "—"
        if len(field_value) > 1024:
            field_value = field_value[:1021] + "…"
        em.add_field(name=f"Group {letter}", value=field_value, inline=True)
        groups_added += 1

    if groups_added == 0:
        em.description = "No group-stage data available yet."

    em.set_footer(text="Updated daily  •  🏆 FIFA World Cup 2026")
    return em

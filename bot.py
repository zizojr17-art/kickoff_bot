...
class UnfollowCompetitionView(discord.ui.View):
    ...
    async def _on_unfollow(self, i: discord.Interaction) -> None:
        ...
        following = dict(state.get_user_following(self.uid))
        following["competitions"] = [c for c in following.get("competitions", []) if c != code]
        state.set_user_following(self.uid, following)
        self._rebuild()
        em = _build_competitions_embed(self.uid)
        await i.response.edit_message(embed=em, view=self if following.get("competitions") else None)


def _build_competitions_embed(uid: str) -> discord.Embed:
    following = state.get_user_following(uid)
    comps = following.get("competitions", [])
    em = discord.Embed(title="🏆 My Followed Competitions", color=emb.C_GOLD)
    if comps:
        em.description = "\n".join(f"• {c}" for c in comps)
    else:
        em.description = (
            "You're not following any competitions yet.\n"
            "Use `/followcompetition WC` to follow the World Cup."
        )
    em.set_footer(text="Only WC (FIFA World Cup 2026) is supported by this bot")
    return em
...
async def slash_dashboard(interaction: discord.Interaction):
    # Defer immediately — channel.send() can be slow enough to time out the interaction
...
_register("Match Commands", "matchtoday", "Today's World Cup matches (alias for /today)")

@tree.command(name="matchtoday", description="Today's World Cup matches — optionally filter by competition code")
@app_commands.describe(competition="Competition code — only WC is supported (leave blank for all)")
async def slash_matchtoday(interaction: discord.Interaction, competition: str = ""):
    await _safe_defer(interaction)
    try:
        matches = await get_todays_matches()
    except Exception as e:
        log.error("[CMD] /matchtoday error: %s", e)
        await interaction.followup.send(
            embed=_error_embed("Failed to load today's matches.", "Try again in a moment.")
        )
        return
...
@bot.command(name="matchtoday", help="Today's World Cup matches. Usage: !matchtoday [WC]")
async def prefix_matchtoday(ctx, competition: str = ""):
    async with ctx.typing():
        matches = await get_todays_matches()
    if competition:
        matches = [m for m in matches if m.get("competition", {}).get("code") == competition.upper()]
    for em in emb.embed_today(matches):
        await ctx.send(embed=em)
...
_register("Match Commands", "competitions", "List supported competition codes")

@tree.command(name="competitions", description="List supported competition codes for this bot")
async def slash_competitions(interaction: discord.Interaction):
    await interaction.response.send_message(embed=emb.embed_competitions(), ephemeral=True)
...
_register("Following", "followcompetition", "Follow a competition (WC supported)")

@tree.command(name="followcompetition", description="Follow a competition — use code WC for the World Cup")
@app_commands.describe(code="Competition code — e.g. WC")
async def slash_followcompetition(interaction: discord.Interaction, code: str):
    uid      = str(interaction.user.id)
    following = dict(state.get_user_following(uid))
    comps    = list(following.get("competitions", []))
    clean    = code.strip().upper()

    if clean != "WC":
        await interaction.response.send_message(
            embed=_error_embed(
                f"`{clean}` is not available — this bot only tracks the World Cup.",
                "Use `/followcompetition WC` to follow FIFA World Cup 2026."
            ),
            ephemeral=True,
        )
        return
    ...
    if clean in comps:
        await interaction.response.send_message(
            embed=discord.Embed(description=f"🏆 You're already following **{clean}**.", color=emb.C_ORANGE),
            ephemeral=True,
        )
        return
    ...
    state.set_user_following(uid, following)
    await interaction.response.send_message(
        embed=_success_embed(f"Now following **{clean}** — FIFA World Cup 2026! ⚽"),
        ephemeral=True,
    )


@bot.command(name="followcompetition", help="Follow a competition. Usage: !followcompetition WC")
async def prefix_followcompetition(ctx, code: str = "WC"):
    uid      = str(ctx.author.id)
    following = dict(state.get_user_following(uid))
    comps    = list(following.get("competitions", []))
    clean    = code.strip().upper()
    if clean in comps:
        await ctx.send(embed=discord.Embed(description=f"Already following **{clean}**.", color=emb.C_ORANGE))
        return
    comps.append(clean)
    following["competitions"] = comps
    state.set_user_following(uid, following)
    await ctx.send(embed=_success_embed(f"Now following **{clean}**."))


# ── /unfollowcompetition ──────────────────────────────────────────────────────

_register("Following", "unfollowcompetition", "Unfollow a competition")

@tree.command(name="unfollowcompetition", description="Unfollow a competition")
@app_commands.describe(code="Competition code to unfollow — e.g. WC")
async def slash_unfollowcompetition(interaction: discord.Interaction, code: str):
    ...
    following["competitions"] = [c for c in comps if c != clean]
    state.set_user_following(uid, following)
    await interaction.response.send_message(
        embed=_success_embed(f"Unfollowed **{clean}**."),
        ephemeral=True,
    )


@bot.command(name="unfollowcompetition", help="Unfollow a competition. Usage: !unfollowcompetition WC")
async def prefix_unfollowcompetition(ctx, code: str = "WC"):
    uid      = str(ctx.author.id)
    following = dict(state.get_user_following(uid))
    following["competitions"] = [c for c in following.get("competitions", []) if c != code.upper()]
    state.set_user_following(uid, following)
    await ctx.send(embed=_success_embed(f"Unfollowed **{code.upper()}**."))


# ── /mycompetitions ───────────────────────────────────────────────────────────

_register("Following", "mycompetitions", "See the competitions you're following")
...
async def slash_mycompetitions(interaction: discord.Interaction):
    ...
    await interaction.response.send_message(
        embed=_build_competitions_embed(uid), view=view, ephemeral=True
    )


@bot.command(name="mycompetitions", help="See competitions you're following")
async def prefix_mycompetitions(ctx):
    uid = str(ctx.author.id)
    await ctx.send(embed=_build_competitions_embed(uid))
...
_register("Admin", "checkapi", "Diagnose the football-data.org API connection and show live data status")
...
async def slash_checkapi(interaction: discord.Interaction):
    ...
    if not key:
        ...
        em.add_field(
            ...
            value=(
                ...
                "Add it in the Replit Secrets panel, then restart the bot.\n"
    ...
    # 2 — Ping the competitions endpoint

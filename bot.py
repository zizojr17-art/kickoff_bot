...
full diagnostics suite, MOTM testing tools, match numbering.
"""
...
def _annotate_match(match: dict) -> dict:
    """Return a copy of the match dict with a 'matchNumber' key injected."""
    m = dict(match)
...
async def slash_team(interaction: discord.Interaction, name: str):
    ...
    try:
        ...
        recent, upcoming_m = [], []
    await interaction.followup.send(embed=emb.embed_team(team, recent, upcoming_m))
...
async def prefix_team(ctx, *, name: str):
    ...
    await ctx.send(embed=emb.embed_team(team, recent, upcoming_m))
...
async def slash_mytimezone(interaction: discord.Interaction):
    uid = str(interaction.user.id)
...
async def slash_testnotifications(interaction: discord.Interaction, notification_type: str = "goal"):
    ...
    if not isinstance(ch, (discord.TextChannel, discord.Thread)):
        await interaction.followup.send(
            embed=_error_embed("Live alerts channel not found — it may have been deleted.", "Run `/setchannel` again."),
            ephemeral=True,
        )
        return
...
_register("Info", "matchfacts", "Detailed facts & events for a finished match")

@tree.command(name="matchfacts", description="Full facts & events for a World Cup match (by match ID)")
@app_commands.describe(match_id="The numeric match ID (get it from /schedule or /today)")
async def slash_matchfacts(interaction: discord.Interaction, match_id: int):
    ...
    em = discord.Embed(
        title=f"📋 Match Facts — ID {match_id}",
...
async def slash_tournament_stats(interaction: discord.Interaction):
    ...
    if mgm:
        ...
        em.add_field(
            ...
            value=f"{h_f} {h_t} {h_s}–{a_s} {a_t} {a_f}",
            inline=False,
        )

...
class UserSettingsView(discord.ui.View):
    ...
    async def reset_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        # Build a fresh view so the select options reflect the reset state
    ...
    async def btn_today(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not embeds:
            await interaction.followup.send(
                embed=discord.Embed(description="📭 No matches scheduled today.", color=emb.C_GREY),
                ephemeral=True,
            )
            return
...
async def broadcast(embed: discord.Embed, reactions: list[str] | None = None,
    ...
    for gid, cfg in state.all_guild_configs().items():
        ...
        try:
            ...
            log.warning("[WARN] No permission to send to channel %s (guild %s)", cid, gid)
...
async def _send_embeds(target, embed_list: list[discord.Embed]) -> None:
    if not embed_list:
        # Guard: never leave a deferred interaction hanging with no response
        no_data = discord.Embed(description="📭 No data available.", color=0x99AAB5)
        if isinstance(target, discord.Interaction):
            await target.followup.send(embed=no_data)
        else:
            await target.send(embed=no_data)
        return
...
async def monitor_loop() -> None:
    ...
    for match in matches:
        ...
        if status in ("SCHEDULED", "TIMED") and ko_dt:
            ...
            # 15-min reminder.
            # bot.py always passes `minutes=15` correctly.
            # ─── embeds.py FIX REQUIRED ───────────────────────────────────────
            # If your reminder still says "1 hour", open embeds.py and find
            # embed_reminder(match, minutes).  Make sure the function body uses
            # the `minutes` parameter, e.g.:
            #   label = f"⏰ Kick-off in {minutes} minute{'s' if minutes != 1 else ''}"
            # NOT a hardcoded string like "in 1 hour".
            # ──────────────────────────────────────────────────────────────────
...
async def _process_fulltime(match: dict, detail: dict | None) -> None:
    ...
    if detail is None:
        log.warning("[MATCH] FT for %s: match detail unavailable, using basic match data", mid)

    # 1. FT embed → channel_id (all modes)
    # Use match as fallback if detail is None so embed functions don't crash
...
async def on_ready() -> None:
    ...
    # Guard against double-start on Discord reconnect (on_ready fires again)
...
async def slash_followteam(interaction: discord.Interaction, name: str):
    uid       = str(interaction.user.id)
    _raw      = _get_user_following(uid)
    # Deep-copy both lists so we never mutate state before save()
    teams     = list(_raw.get("teams", []))
    comps     = list(_raw.get("competitions", []))
    ...
    teams.append(clean)
    _set_user_following(uid, {"teams": teams, "competitions": comps})
...
async def on_command_error(ctx, error):
    ...
    if isinstance(error, commands.MissingPermissions):
        await ctx.send(embed=discord.Embed(
            description="🔒 You don't have permission to run this command.", color=emb.C_RED
        ))

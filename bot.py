...
class MotmVoteView(discord.ui.View):
    ...
    def __init__(self, match_id: str, nominees: list[str]):
        ...
        self.match_id = match_id
        options = [discord.SelectOption(label=n[:100], value=n[:100]) for n in nominees[:10]]
...
async def broadcast_leaderboard(guild_id: str, cfg: dict) -> None:
    """Send updated leaderboard to guild's leaderboard_channel_id; pin newest, unpin previous."""
    cid = cfg.get("leaderboard_channel_id")
    if not cid:
        return
    channel = bot.get_channel(int(cid))
    if not isinstance(channel, (discord.TextChannel, discord.Thread)):
...
def _get_motm_nominees(detail: dict) -> list[str]:
    """Build MOTM nominee list from goals, assists, and lineup data."""
...
async def monitor_loop() -> None:
    ...
    for match in matches:
        ...
        if status in ("SCHEDULED", "TIMED") and ko_dt:
            ...
            # Lineups: 55–70 min before kickoff (replaces 60-min reminder)
...
async def _process_fulltime(match: dict, detail: dict) -> None:
    ...
    if h is not None and a is not None:
        ...
        # Award points in every guild's leaderboard
    ...
    for gid, cfg in state.all_guild_configs().items():
        ...
        # Send result to results_channel
        cid = cfg.get("results_channel_id")
        if cid:
            ch = bot.get_channel(int(cid))
    ...
    for gid, cfg in state.all_guild_configs().items():
        if state.get_leaderboard(gid):
            await broadcast_leaderboard(gid, cfg)
...
async def _post_delayed_recap(match: dict, delay: int) -> None:
    ...
    await broadcast(emb.embed_full_recap(match, detail, youtube), min_mode="detailed")
...
async def daily_summary_loop() -> None:
    ...
    for gid, cfg in configs.items():
        ...
        try:
            ...
            # Group tables after daily summary (detailed mode only)
...
async def _post_group_tables(guild_id: str, cfg: dict) -> None:
    ...
    # Send one embed per group letter
    last_msg = None
...
async def _post_commands_menu_if_needed() -> None:
    ...
    for gid, cfg in state.all_guild_configs().items():
        ...
        # Verify existing message still exists
        if existing_id:
            try:
...
async def on_ready() -> None:
    ...
    # Re-register persistent ModePicker view so button interactions survive restarts
...
@tree.command(name="setleaderboardchannel", description="Set this channel for the pinned leaderboard")
@app_commands.default_permissions(manage_channels=True)
async def slash_setleaderboardchannel(interaction: discord.Interaction):
    state.set_channel_id(str(interaction.guild_id), "leaderboard_channel_id", str(interaction.channel_id))
    await interaction.response.send_message(embed=_channel_set_embed(interaction.channel, "leaderboard"))


@bot.command(name="setleaderboardchannel", help="Set leaderboard channel")
@commands.has_permissions(manage_channels=True)
async def prefix_setleaderboardchannel(ctx):
    state.set_channel_id(str(ctx.guild.id), "leaderboard_channel_id", str(ctx.channel.id))
    await ctx.send(embed=_channel_set_embed(ctx.channel, "leaderboard"))
...
def _status_embed(guild: discord.Guild | None) -> discord.Embed:
    ...
    em.add_field(name="📡 Match events",     value=_ch("channel_id"),             inline=True)
    ...
    em.add_field(name="🥇 Leaderboard",      value=_ch("leaderboard_channel_id"), inline=True)
    em.add_field(name="🎛️ Commands menu",    value=_ch("commands_channel_id"),    inline=True)
    ...
    em.add_field(name="Tracked matches",
                 value=f"{all_tracked} matches  ·  {all_snaps} with live snapshots",
                 inline=False)
...
async def slash_testembed(interaction: discord.Interaction, embed_type: str):
    await interaction.response.send_message(embed=emb.embed_test(embed_type), ephemeral=True)
...
async def on_command_error(ctx, error):
    ...
    if isinstance(error, commands.MissingPermissions):
        ...
        await ctx.send(embed=discord.Embed(
            description=f"⚠️ Missing argument — see `!help`.", color=emb.C_ORANGE
        ))
    else:

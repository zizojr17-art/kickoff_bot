...
- YouTube highlight search (no API key required)
"""
import aiohttp
import asyncio
import json
...
async def get_match_detail(match_id) -> Optional[dict]:
    """Fetch full match detail (includes goals, bookings, lineups, stats)."""
...
async def load_wc_match_order() -> None:
    ...
    if data:
        ...
        log.warning("[API] Could not load WC 2026 match order")
    _wc_match_order_loaded = True


def wc_match_number(match_id) -> Optional[int]:
...
async def get_team(team_id) -> Optional[dict]:
    return await fetch(f"/teams/{team_id}")


async def get_team_matches(team_id, *, status: str = "FINISHED", limit: int = 5) -> list[dict]:
...
def has_confirmed_lineups(detail: dict) -> bool:
    """Return True if the match detail contains confirmed starting lineups."""
    if not detail:
        return False
    home_lu = (detail.get("homeTeam") or {}).get("lineup") or []
    away_lu = (detail.get("awayTeam") or {}).get("lineup") or []
    return len(home_lu) >= 11 and len(away_lu) >= 11


def get_lineup(detail: dict, side: str) -> tuple[list[dict], str]:
    """Return (starters_list, formation) for 'homeTeam' or 'awayTeam'."""
    team = detail.get(side) or {}
    starters = (team.get("lineup") or [])[:11]
...
async def search_youtube_highlights(query: str) -> Optional[dict]:
    """
    Search YouTube for match highlights without an API key.
    Parses ytInitialData from the search results page.
    Returns {title, url, channel, thumbnail} or None.
    """
    encoded = urllib.parse.quote_plus(f"{query} highlights")
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(url, headers=headers, timeout=aiohttp.ClientTimeout(total=12)) as resp:
                if resp.status != 200:
                    log.warning("[API] YouTube search HTTP %s for '%s'", resp.status, query)
                    return None
                text = await resp.text()

        # YouTube embeds video data as JSON in a script tag
        match = re.search(r'var ytInitialData\s*=\s*(\{.*?\});\s*</script>', text, re.DOTALL)
        if not match:
            log.warning("[API] ytInitialData not found for '%s'", query)
            return None
        ...
        for section in contents:
            ...
            for item in items:
                ...
                if vid_id and title:
                    log.info("[API] YouTube found '%s' for query '%s'", title, query)
                    return {
                        "video_id": vid_id,
                        "title":    title,
        ...
        log.warning("[API] YouTube search failed for '%s': %s", query, e)
    return None
...
def get_score(match: dict, phase: str = "fullTime") -> tuple:
    block = (match.get("score") or {}).get(phase) or {}
    return block.get("home"), block.get("away")


def get_current_score(match: dict) -> tuple:
...
def penalty_scores(match: dict) -> tuple:
...
def goal_key(goal: dict) -> str:
    ...
    return f"{goal.get('minute', '?')}_{s_id}_{goal.get('type', 'REGULAR')}"


def card_key(booking: dict) -> str:
    player = booking.get("player") or {}
    p_id   = player.get("id") or player.get("name") or "unknown"
    return f"{booking.get('minute', '?')}_{p_id}_{booking.get('card', 'RED')}"
